				...
				if (args.length == 0) {
					...
					if (args.length == 1) {
						...
						if (positionsManager.exists(name)) {
							...
							// TODO: Add config for changing messages
						...
						if (args[0].equalsIgnoreCase("delete")) {
							...
							if (positionsManager.exists(name)) {
								...
								Component textComponent = prefix.append(Component.text("Die Position ").color(TextColor.color(0xa8a8a8a)).append(Component.text(name).color(TextColor.color(0x166e00))).append(Component.text(" wurde erfolgreich gelöscht!").color(TextColor.color(0xa8a8a8a))));
								commandSender.sendMessage(textComponent);
								return true;
							} else {
								Component textComponent = prefix.append(Component.text("Die Position ").color(TextColor.color(0xa8a8a8a)).append(Component.text(name).color(TextColor.color(0x166e00))).append(Component.text(" existiert nicht!").color(TextColor.color(0xa8a8a8a))));
								commandSender.sendMessage(textComponent);
								return true;
							}
						}
